...
class Invoice extends React.Component {
    ...
    async componentDidMount() {
        if (this.state.invoice.length === 0) {
            ...
            const response = await fetch(
                "http://localhost:8080/eMart/Product/prodInvoice"
            );
    ...
    async componentWillMount() {
        const response = await fetch(
            "http://localhost:8080/eMart/Product/totalpts"
        );
        const res = await response.json();
        this.setState({ a: res });
        //console.log(arr);
                ...
                <div>
                    ...
                    <div className="container">
                        <div className="row">
                            <div className="col-4">
                                <h4>
                                    <p> Name: </p>
                                    <small>{k.cust_Name}</small>{" "}
                                </h4>
                            </div>
                            <div className="col-4">
                                <h4>
                                    <p>Billing Address:</p> <small>{k.cust_Address}</small>{" "}
                                </h4>
                            </div>
                            <div className="col-4">
                        ...
                        <tbody style={{ textAlign: "center" }}>
                            {cart.map((val) => (
                                <tr>
                                    ...
                                    <td>
                                        <button
                                            onClick={() => removeProduct(val.prodID)}
                    ...
                    <div className="nav justify-content-center">
                        <div class="center-block">
                            ...
                            <Button className="btn btn-success" onClick={this.openCheckout}>
                                Pay
                            </Button>
                            <span>&nbsp;&nbsp;&nbsp;</span>
